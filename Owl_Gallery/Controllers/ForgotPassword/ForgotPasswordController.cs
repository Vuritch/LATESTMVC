...
namespace Owl_Gallery.Controllers.ForgotPassword
{
    public class ForgotPasswordController : Controller
    {
        ...
        public async Task<IActionResult> ResendCode([FromBody] string email)
        {
            ...
            if (user == null) return Json(new { ok = false, error = "User not found." });

            // throttle: only allow every 2‑min
            if (user.CodeSentAt.HasValue && user.CodeSentAt.Value.AddMinutes(2) > DateTime.UtcNow)
            {
                return Json(new { ok = false, error = "Please wait before requesting another code." });
            }
